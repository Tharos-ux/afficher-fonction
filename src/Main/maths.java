...
public class maths {
	...
	public static final String[] listeStringFonctions = { "Carré", "Cube", "Complexe conjugé", "Exponentielle",
			"Inverse", "Cosinus", "Sinus", "Tangente" };
	...
	public static complexe cosinus(complexe z) {
		return new complexe((float) (Math.cos(z.re) * (Math.exp(z.im) + Math.exp(-z.im)) / 2),
				(float) (-Math.sin(z.re) * (Math.exp(z.im) - Math.exp(-z.im)) / 2));
	}

	public static complexe sinus(complexe z) {
		return new complexe((float) (Math.sin(z.re) * (Math.exp(z.im) + Math.exp(-z.im)) / 2),
				(float) (Math.cos(z.re) * (Math.exp(z.im) - Math.exp(-z.im)) / 2));
	}

	public static complexe tangente(complexe z) {
		return new complexe(
				(float) (Math.sin(2 * z.re) / (Math.cos(2 * z.re) + (Math.exp(2 * z.im) + Math.exp(-2 * z.im)) / 2)),
				(float) ((Math.exp(2 * z.im) - Math.exp(-2 * z.im))
						/ (2 * Math.cos(2 * z.re) - Math.exp(2 * z.im) - Math.exp(-2 * z.im))));
	}
